...
export default {
    ...
    scrollableParentNodes () {
      ...
      try {
        ...
        while (parentNode !== null) {
          if (['auto', 'scroll', 'visible'].includes(getComputedStyle(parentNode).overflowY)) {
        ...
        while (parentNode !== null) {
          if (parentNode.classList.contains('el-table')) {
            for (const el of [...this.tableEl.parentElement.children]) {
              ...
              if (computedHeight.endsWith('px')) offsetTop -= parseFloat(computedHeight)
            }

            // nested table
            const computedHeight = getComputedStyle([...parentNode.children].find(el => el.classList.contains('el-table__header-wrapper'))).height
  ...
  watch: {
    ...
    tableEl () {
      ...
      const tableHeader = [...elTable.children].find(el => el.classList.contains('el-table__header-wrapper'))
      if (tableHeader !== undefined) {
        this.tableHeader = tableHeader
      }

      const elTableFixed = [...elTable.children].find(el => el.classList.contains('el-table__fixed'))
      if (elTableFixed !== undefined) {
        this.tableHeaderFixed = [...elTableFixed.children].find(el => el.classList.contains('el-table__fixed-header-wrapper'))
      }

      const elTableFixedRight = [...elTable.children].find(el => el.classList.contains('el-table__fixed-right'))
      if (elTableFixedRight !== undefined) {
        this.tableHeaderFixedRight = [...elTableFixedRight.children].find(el => el.classList.contains('el-table__fixed-header-wrapper'))
      }
    },
    tableHeader () {
    ...
    tableHeaderFixedRight () {
      ...
      const parentHeight = getComputedStyle(this.tableHeaderFixedRight.parentElement).width
      const childHeight = getComputedStyle([...this.tableHeaderFixedRight.children].find(el => el.classList.contains('el-table__header'))).width
